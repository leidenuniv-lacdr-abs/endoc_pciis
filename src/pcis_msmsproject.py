...
class PcisMsmsExperiment:
    ...
    def set_pcis(self, pcis_file: Path):
        self.pcis = pd.read_csv(
            pcis_file, sep="\t", converters={"precursor": str, "product": str}
        )

        # Add column with unique transition information
        self.pcis["transition"] = self.pcis.apply(
            lambda x: f"{x['precursor']}_{x['product']}_{x['polarity']}", axis=1
        )

        # TODO: build support for multiple pcis
        self.pcis = self.pcis.iloc[0]  # use pcis from first line

    def load_data(
        self,
        mzml_file_path: Path,
        scans_per_second: int = 8,
        window_length: int = 5,
        polyorder: int = 3,
    ):
        ...
        self.data = pd.concat(
            [
                df_from_mzml(
                    mzml_file, scans_per_second, window_length, polyorder
                )
    ...
    def apply_pcis(self):
        ...
        pcis_filter = self.data["transition"] == self.pcis["transition"]
        pcis_data = self.data[pcis_filter][["file", "rt", "intensity"]].copy()

        # Change the column name of the intensity to pcis and
        # set the index of the DataFrame to file/rt to be able
        # to stitch the column to the target signal
        pcis_data.rename(columns={"intensity": "pcis"}, inplace=True)
        pcis_data = pcis_data.set_index(["file", "rt"])
        ...
        for tIdx, target in self.targets.iterrows():
            ...
            # Prepare index for join with pcis signal using file/rt
            target_data = target_data.set_index(["file", "rt"])

            # Stitch the PCI_IS signal to the dataframe as a column
            dfs.append(target_data.join(pcis_data["pcis"]).reset_index())
        ...
        # Calculate ratio between intensity and pcis
        target_data["ratio"] = target_data["intensity"] / target_data["pcis"]
    ...
    def apply_rt_correction(self, reference_file: Path):
        ...
        reference_ratio = df[reference__filter][
            ["target", "rt", "ratio"]
        ].copy()
        reference_ratio = reference_ratio.set_index(["target", "rt"])

        # Change the column name
        reference_ratio.rename(
            columns={"ratio": "ratio_reference"}, inplace=True
        )

        # Append a column with the ratio by rt from the reference file
        df_aligned = (
            df.set_index(["target", "rt"])
            .join(reference_ratio)
            .reset_index()
            .dropna()
        )
        ...
        for gIdx, df_grouped in df_aligned.groupby(["target", "file"]):
            df_grouped["ratio_aligned"] = shift(  # apply phase shift
                df_grouped["ratio"],
                float(
                    phase_align(  # calculate phase shift
                        df_grouped["ratio_reference"].values,
                        df_grouped["ratio"].values,
                        res=1,
                    )
                ),
                mode="constant",
        ...
        for tIdx, df_target in self.data.groupby("target"):
            ...
            for fIdx, df_target_file in df_target.groupby("file"):
                ...
                for signal_type in ["intensity", "ratio", "ratio_aligned"]:
                    ...
                    peak_rt_start = df_target_file.iloc[peak_width_index[2],][
                        "rt"
                    ]
                    peak_rt_end = df_target_file.iloc[peak_width_index[3],][
                        "rt"
                    ]
                    peak_width = [
                        pw[0] - pw[1] for pw in zip(peak_rt_end, peak_rt_start)
                    ]
    ...
    def plot_results(
        self, rt_window: int = 12, width: int = 210, height: int = 180
    ):
        ...
        for tIdx, target in self.targets.iterrows():
            ...
            for concentration in self.data["concentration"].unique():
                ...
                # Original signal
                intensity_plot = (
                    alt.Chart(df_concentration_signal)
                    .mark_line()
                    .encode(x="rt", y="intensity", color="sample")
                    .properties(
                        width=width,
                        height=height,
                ...
                intensity_plot = intensity_plot + alt.Chart(
                    df_concentration_signal
                ).mark_line(strokeWidth=0.5).encode(
                    x="rt", y="pcis", color="sample"
                ).properties(
                ...
                intensity_plot = intensity_plot + alt.Chart(
                    df_concentration_peaks[
                        df_concentration_peaks["signal_type"] == "intensity"
                    ]
                ).mark_rule(color="red", strokeWidth=2).encode(
                    alt.X("mean(peak_rt_apex)", title="rt")
                )
                ...
                ).mark_rule(color="grey", strokeWidth=2).encode(
                    x="min(peak_rt_start)"
                )
                ...
                unaligned_plot = (
                    alt.Chart(df_concentration_signal)
                    .mark_line()
                    .encode(x="rt", y="ratio", color="sample")
                    .properties(
                        width=width,
                        height=height,
                ...
                unaligned_plot = unaligned_plot + alt.Chart(
                    df_concentration_peaks[
                        df_concentration_peaks["signal_type"] == "ratio"
                    ]
                ).mark_rule(color="red", strokeWidth=2).encode(
                    alt.X("mean(peak_rt_apex)", title="rt")
                )
                ...
                aligned_plot = (
                    alt.Chart(df_concentration_signal)
                    .mark_line()
                    .encode(x="rt", y="ratio_aligned", color="sample")
                    .properties(
                        width=width,
                        height=height,
                ...
                aligned_plot = aligned_plot + alt.Chart(
                    df_concentration_peaks[
                        df_concentration_peaks["signal_type"] == "ratio_aligned"
                    ]
                ).mark_rule(color="red", strokeWidth=2).encode(
                    alt.X("mean(peak_rt_apex)", title="rt")
                )
                ...
                plots = alt.vconcat(
                    plots, ((intensity_plot | unaligned_plot | aligned_plot))
                )
            ...
            for signal_type in ["intensity", "ratio_aligned"]:
                ...
                peaks_filter = (
                    f"target == {tIdx} and signal_type == '{signal_type}'"
                )
                ...
                error_points = (
                    ...
                    .encode(
                        ...
                        y=alt.Y(
                            "concentration:N", sort=alt.SortField("sample")
                        ),
                    ...
                    .properties(
                        width=width * 1.5,
                        height=height / 2,
                        title=f"{target['name']}: mean area + error bars (based on: {signal_type})",
                    )
                )
